...
if not GOOGLE_API_KEY or not MONGODB_URI:
    st.error("âŒ Faltan GOOGLE_API_KEY o MONGODB_URI en secrets")
...
def crear_indice_vectorial():
  ...
  if "vector_index" in existing_indexes:
    print("El Ã­ndice 'vector_index' ya existe. No se crea nuevamente.")
    return
...
def procesar_pdf(archivo_pdf, nombre_pdf):
    ...
    st.info("ðŸ“¤ Subiendo PDF a Backblaze...")
...
def obtener_pdf(nombre_pdf):
    """Devuelve la URL pÃºblica o autenticada del PDF en Backblaze."""
...
Responde en espaÃ±ol, de forma clara.
"""
...
archivo_pdf = st.file_uploader("ðŸ“¤ Sube un PDF", type=["pdf"])
...
if archivo_pdf:
    if st.button("Procesar y guardar PDF"):
        ...
        st.info("ðŸ“– Vista previa del PDF desde Backblaze:")
...
st.subheader("ðŸ’¬ Pregunta sobre el contenido del PDF")
...
if pregunta:
    with st.spinner("Buscando en el PDF..."):
        ...
        if not similares:
            respuesta = "No encontrÃ© informaciÃ³n relevante."
        else:
